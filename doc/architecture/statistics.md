...
**Test coverage: 0%.** There is no test framework configured, no `test/` directory, and the `npm test` step in CI is commented out. This is the single largest quality risk in the project — see [Errors & Weaknesses § E1](errors-and-weaknesses.md#e1-zero-automated-tests).

Coverage that **could** be achieved if tests were added (rough estimates per [Refactoring R6](recommendations-for-refactoring.md#r6-introduce-a-test-suite-l)):
...
If [R6 (test suite)](recommendations-for-refactoring.md#r6-introduce-a-test-suite-l) gets to even 40% coverage, the index would land around **70/100** — a meaningful step. Adding [R1 (file splitting)](recommendations-for-refactoring.md#r1-extract-the-per-device-type-input-parsers-into-one-file-each-m) probably brings function-size into the 8/10 range, lifting another 4 points.

...
The records are stored individually in [`adr/`](adr/). They use a lightweight Nygard-style format (Title / Status / Context / Decision / Consequences). Numbering is chronological-ish — earlier numbers reflect older decisions, but the dates aren't always recoverable from git history because some decisions predate this documentation effort.

## Index

|                                                  # | Title                                                                     | Status   |
| -------------------------------------------------: | ------------------------------------------------------------------------- | -------- |
|  [001](adr/001-three-node-types-per-generation.md) | Three node-type pairs, one per protocol generation                        | Accepted |
|     [002](adr/002-config-driven-device-catalog.md) | Device catalog as data, not code                                          | Accepted |
|        [003](adr/003-polling-vs-callback-modes.md) | Two operating modes: polling and callback                                 | Accepted |
|    [004](adr/004-gen2-scripts-vs-gen1-webhooks.md) | Callbacks via uploaded scripts (gen 2+) vs. built-in webhooks (gen 1)     | Accepted |
|         [005](adr/005-cloud-rate-limited-axios.md) | Cloud node uses a shared rate-limited axios instance                      | Accepted |
|             [006](adr/006-blu-via-gen2-gateway.md) | BLU devices addressed through a gen 2+ gateway, not directly              | Accepted |
| [007](adr/007-server-config-node-owns-listener.md) | The fastify callback listener lives on the server config node             | Accepted |
